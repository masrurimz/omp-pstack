...
4. Pick the worker model from `swarm workers` in `~/.omp/agent/config.yml modelRoles` when present. Otherwise use `your smol-role model`. For a model race, name each arm's model up front.
