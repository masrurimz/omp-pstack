...
- `model`: your configured how-explorer model (default `your smol-role model`)
- `readonly`: `true`
...
- dispatch via the OMP `task` tool (default worker)
- `model`: your configured how-explainer model (default `your slow-role model`)
- `readonly`: `true`
...
After the explanation is complete, spawn one architectural critic per model in your configured how-critics list (defaults `your slow-role model`, `your default-role model`, `your smol-role model`, `your task-role model`), all in a single message.
