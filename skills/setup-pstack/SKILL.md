...
The default role-to-model mapping is the rule shape shown in step 5 below. If `~/.omp/agent/config.yml modelRoles` already exists, read it and treat its `# budget` line and its role values as the current choices. Otherwise start from those defaults. A line whose role is not in step 5, such as `how critics`, is from a retired role. Drop it.
...
arena runners: claude-opus-5-5-max, your default-role model, grok-4.7-xhigh-fast
arena cross-judge pool: claude-opus-5-5-max, your default-role model, grok-4.7-xhigh-fast
swarm workers: grok-4.7-xhigh-fast
architect runners: claude-opus-5-5-max, your default-role model, grok-4.7-xhigh-fast
interrogate reviewers: claude-opus-5-5-max, your default-role model, grok-4.7-xhigh-fast
```
